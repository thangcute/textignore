...
namespace OOS.GHR.Services.Services.Task
{
    public class ProjectStatusService
    {
        ...
        public static async Task<(List<TSK_DuAn_TrangThai> rs, int total)> get(string _conditions = "", int _limit = 15, int _offset = 0, bool _all = false)
        {
            int _total = 0;
            List<TSK_DuAn_TrangThai> _rs = new List<TSK_DuAn_TrangThai>();

            try
            {
                using (var db = new OosContext())
                {
                    string sql = "SELECT * FROM [dbo].[TSK_DuAn_TrangThai]";
                    if (!string.IsNullOrEmpty(_conditions))
                        sql = string.Format("{0} WHERE {1}", sql, _conditions);
                    var _data = db.TSK_DuAn_TrangThai.SqlQuery(sql);
                    if (_data.Any())
                    {
                        _total = _data.Count();
                        if (_all)
        ...
        public static async Task<List<CategoryModel>> list()
        {
            List<CategoryModel> rs = new List<CategoryModel>();
            using (var db = new OosContext())
            {
                var data = db.TSK_DuAn_TrangThai.AsEnumerable().Where(x => x.Status.GetValueOrDefault(0) == 1);

                if (data != null && data.Any())
                    rs = data.OrderBy(x => x.STT)
        ...
        public static async Task<TSK_DuAn_TrangThai> detail(int id = 0)
        {
            TSK_DuAn_TrangThai rs = new TSK_DuAn_TrangThai();
            using (var db = new OosContext())
            {
                rs = await db.TSK_DuAn_TrangThai.FindAsync(id);
            }
            return rs;
        }

        public static async Task<bool> post(TSK_DuAn_TrangThai model)
        {
            bool rs = false;
            using (var db = new OosContext())
            {
                db.TSK_DuAn_TrangThai.Add(model);
                rs = await db.SaveChangesAsync() > 0 ? true : false;
            }
            return rs;
        }

        public static async Task<bool> put(int id = 0, TSK_DuAn_TrangThai model = null)
        {
            bool rs = false;
            using (var db = new OosContext())
            {
                db.TSK_DuAn_TrangThai
        ...
        public static async Task<bool> delete(int id = 0)
        {
            ...
            using (var db = new OosContext())
            {
                db.TSK_DuAn_TrangThai.RemoveRange(db.TSK_DuAn_TrangThai.Where(x => x.TrangThaiID == id));
                rs = await db.SaveChangesAsync() > 0 ? true : false;
            }
            return rs;
        }
